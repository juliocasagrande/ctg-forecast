...
// Acesso de página/botão é fail-closed por padrão (ver middleware/auth.js): sem
// grupo, o usuário não vê/acessa nada. Para não travar quem acabou de ser criado
// numa sidebar vazia, atribuímos automaticamente o grupo de permissão padrão do
// cargo escolhido (ver permission_groups.default_for_role) — o admin pode
// adicionar/trocar grupos depois pela aba "Grupos de Permissão".
async function assignDefaultGroupForRole(userId, role) {
  await pool.query(
...
router.post('/:id/approve', requireRole('admin'), async (req, res) => {
  try {
    const r = await pool.query(
      `UPDATE users SET pending_approval = false, active = true, updated_at = NOW() WHERE id = $1 RETURNING role`,
      [req.params.id]
    );
    if (r.rows[0]) await assignDefaultGroupForRole(parseInt(req.params.id), r.rows[0].role);
...
router.get('/', requireRole('admin', 'planejador'), async (req, res) => {
  try {
    const r = await pool.query(`
      ...
      LEFT JOIN permission_groups g ON g.id = upg.group_id
      GROUP BY u.id
...
router.post('/', requireRole('admin'), async (req, res) => {
  try {
    ...
    await assignDefaultGroupForRole(r.rows[0].id, userRole);
