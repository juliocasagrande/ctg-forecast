...
function RoleBadge({ role }) {
  return (
    <span style={{ fontSize: '0.72rem', fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: ROLE_COLORS[role] + '18', color: ROLE_COLORS[role] }}>
      {ROLE_LABELS[role]}
    </span>
  );
}
...
const EMPTY_USER = { name: '', email: '', password: '', role: 'engenheiro' };
...
export default function AdminPanel() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingUser) {
        const r = await api.put(`/users/${editingUser.id}`, form);
    ...
    <div>
      ...
      <div className="stat-grid" style={{ marginBottom: 20 }}>
        {[
          ...
          { label: 'Administradores', value: stats.admins, color: '#001F5B' },
          { label: 'Gestores', value: stats.gestores, color: '#0070B8' },
        ...
        <div>
          {pending.length === 0 ? (
            ...
            <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '14px 16px', marginBottom: 10 }}>
              ...
              <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginRight: 8 }}>
      ...
      {activeTab === 'users' && (
        <div>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <select className="form-select" value={filterRole} onChange={e => setFilterRole(e.target.value)} style={{ maxWidth: 180, marginBottom: 0 }}>
          ...
          <div className="card">
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              <thead>
                <tr>
                  {['Usuário', 'E-mail', 'Perfil', 'Projetos', 'Status', ''].map(h => (
