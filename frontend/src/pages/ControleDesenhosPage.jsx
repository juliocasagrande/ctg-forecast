...
/* ─── Constants ──────────────────────────────────────────────────────────────── */
const DRAWING_TYPES = [
  { value: 'DES', label: 'Desenho Técnico' },
];
const AREAS = [
...
const TYPE_META    = Object.fromEntries(DRAWING_TYPES.map(t => [t.value, t]));
              ...
              {slices.map((s,i) => {
                ...
                return (
                  <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={s.color} strokeWidth={14}
                    ...
                    onMouseEnter={e => { setHovered(s); setTooltipPos({ x:e.clientX, y:e.clientY }); }}
...
function StatusModal({ open, onClose, onSaved, doc }) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await api.patch(`/drawings/${doc.id}/status`, {
        status,
      ...
      <div className="modal" style={{ maxWidth:440, width:'92vw' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body" style={{ display:'flex', flexDirection:'column', gap:14 }}>
          <div style={{ fontSize:'0.78rem', color:'#64748B', background:'#F8FAFC', padding:'8px 12px', borderRadius:8 }}>
            Desenho: <strong style={{ fontFamily:'monospace', color:'#001F5B' }}>{doc?.code}</strong>
          </div>
...
/* ─── RevisionModal — nova revisão (só data e solicitante) ──────────────────── */
function RevisionModal({ open, onClose, onSaved, doc, allUsers }) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await api.post(`/drawings/${doc.id}/revision`, { date, responsible });
      ...
      <div className="modal" style={{ maxWidth:500, width:'92vw' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body" style={{ display:'flex', flexDirection:'column', gap:14 }}>
          <div style={{ background:'#EFF6FF', border:'1px solid #BFDBFE', borderRadius:8, padding:'10px 14px', fontSize:'0.8rem', color:'#1D4ED8' }}>
            ℹ️ Criando <strong>Revisão R{nextRev}</strong> de <strong style={{ fontFamily:'monospace' }}>{doc.base_code || doc.code}</strong>. O desenho original é mantido.
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>
            ...
            <Field label="Assunto"><input value={doc.subject} readOnly style={fLocked} title={doc.subject}/></Field>
          </div>
...
/* ─── DrawingModal — criar / editar ─────────────────────────────────────────── */
function DrawingModal({ open, onClose, onSaved, doc, nextSeq, allUsers }) {
  const { toast } = useToast();
  const isEdit    = !!doc;
  ...
  async function handleSubmit() {
    ...
    try {
      if (isEdit) {
        await api.put(`/drawings/${doc.id}`, form);
        toast('Desenho atualizado com sucesso!', 'success');
      } else {
        await api.post('/drawings', form);
        toast('Desenho registrado com sucesso!', 'success');
      }
      onSaved();
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth:720, width:'95vw', maxHeight:'93vh', display:'flex', flexDirection:'column' }} onClick={e => e.stopPropagation()}>
        <div className="modal-header" style={{ flexShrink:0 }}>
          <span className="modal-title">{isEdit ? '✏️ Editar Desenho' : '📐 Novo Desenho'}</span>
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,0.7)', fontSize:'1.1rem' }}>✕</button>
        </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
            ...
            <Field label="Revisão">
              <input type="number" value={form.revision} onChange={e => set('revision', e.target.value)}
                min={0} placeholder="R0" style={isEdit ? fLocked : fS} readOnly={isEdit}/>
            </Field>
          </div>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {STATUSES.map(s => (
                <label key={s.value} style={{
                  ...
                  <input type="radio" name="draw_status" value={s.value} checked={form.status===s.value}
        ...
        <div className="modal-footer" style={{ flexShrink:0 }}>
          ...
          <button className="btn btn-primary" onClick={handleSubmit} disabled={saving}>
            {saving ? 'Salvando...' : isEdit ? '💾 Salvar Alterações' : '📐 Registrar Desenho'}
          </button>
        </div>
      </div>
    </div>
...
function VBarChart({ data, title, activeFilter, onFilter }) {
  ...
  return (
    <div style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:10, borderTop:'3px solid #0066B3', padding:'14px 16px', flex:1, minWidth:0, width:'100%' }}>
      ...
      {hovered && hovered.tooltipBreakdowns && (
        <ChartTooltip pos={tooltipPos} color={hovered.color} label={hovered.fullName || hovered.label} value={hovered.value} breakdowns={hovered.tooltipBreakdowns} />
      )}
    </div>
  );
}
...
function generateHTMLReport(docs, stats, year) {
  ...
  const typeRows = DRAWING_TYPES.map(t => {
...
<title>Relatório de Controle de Desenhos — CTG Brasil ${year}</title>
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;font-size:13px;color:#1E293B;background:#F8FAFC}.page{max-width:1200px;margin:0 auto;padding:32px 24px}.header{padding:24px 28px;background:#001F5B;color:#fff;border-radius:12px;margin-bottom:28px}.header h1{font-size:1.4rem;font-weight:700}.header .sub{font-size:0.85rem;opacity:.7;margin-top:4px}.section{margin-bottom:28px}.section h2{font-size:.75rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#64748B;margin-bottom:14px;padding-bottom:6px;border-bottom:2px solid #E2E8F0}.stats-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:14px}.stat-card{background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:16px;border-top:3px solid #0066B3}.stat-card .num{font-size:2rem;font-weight:700;color:#0066B3;line-height:1}.stat-card .lbl{font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94A3B8;margin-bottom:6px}table{width:100%;border-collapse:collapse;background:#fff;border-radius:10px;overflow:hidden;border:1px solid #E2E8F0;font-size:.83rem}th{background:#001F5B;color:#fff;padding:10px 12px;text-align:left;font-size:.72rem;text-transform:uppercase;letter-spacing:.06em}td{padding:9px 12px;border-bottom:1px solid #F1F5F9}.tables-row{display:grid;grid-template-columns:1fr 1fr;gap:20px}.footer{text-align:center;font-size:.72rem;color:#94A3B8;margin-top:32px;padding-top:16px;border-top:1px solid #E2E8F0}@media print{body{background:#fff}.page{padding:20px}}</style>
  ...
  <div class="header"><div class="sub">CTG Brasil — Engenharia de Manutenção</div><h1>Relatório de Controle de Numeração de Desenhos</h1><div class="sub">Gerado em ${now} · Total: ${docs.length} desenhos</div></div>
  ...
  <div class="tables-row section">
    <div><h2>Desenhos por Tipo</h2><table><thead><tr><th>Sigla</th><th>Tipo</th><th>Qtd</th></tr></thead><tbody>${typeRows}</tbody></table></div>
    <div><h2>Desenhos por Status</h2><table><thead><tr><th>Status</th><th>Qtd</th></tr></thead><tbody>${statusRows}</tbody></table></div>
  </div>
  <div class="section"><h2>Lista de Desenhos — ${year}</h2>
    <table><thead><tr><th>Código</th><th>Usina</th><th>Solicitante</th><th>Data</th><th>Assunto</th><th>Status</th><th>Link</th></tr></thead><tbody>${docRows}</tbody></table>
  </div>
...
// Deriva a usina a partir do texto residual do código (ex.: "UHE JUP" → "UHE Jupiá")
function _resolvePlantFromText(text) {
  if (!text) return null;
  const tokens = text.trim().split(/\s+/);
  const sigla = tokens[tokens.length - 1]?.toUpperCase();
  if (!sigla) return null;
  const match = Object.entries(PLANT_SIGLAS).find(([, s]) => s === sigla);
  return match ? match[0] : null;
}

/* ─── ImportDocxModal — importar NUMERAÇÃO DE DESENHOS .docx ────────────────── */
function ImportDocxModal({ open, onClose, onImported, allUsers }) {
  ...
  const parseCode = (raw) => {
    if (!raw) return null;
    const m = raw.trim().match(/^([A-Z]{2,5})-([A-Z]{2,4})-(\d+)-(\d+)(?:-R(\d+))?(?:[\s-]+(.+))?$/);
    if (!m) return null;
    return {
      type: m[1], area: m[2],
  ...
  const handleFile = async (f) => {
    ...
    try {
      ...
      doc.querySelectorAll('table').forEach(table => {
        ...
        const colIdx = {
          code:        headerCells.findIndex(h => /^cod|^n[°º]|desenho/.test(h)),
          responsible: headerCells.findIndex(h => /solicit|respons/.test(h)),
        ...
        trs.forEach((tr, ti) => {
          ...
          const plantName = plantCol
            ? plantCol.split(/[,/;]+/).map(p => p.trim()).filter(Boolean)
            : (_resolvePlantFromText(parsed.plantText) ? [_resolvePlantFromText(parsed.plantText)] : null);

          rows.push({
            type: parsed.type, area: parsed.area,
            sequence_number: parsed.sequence_number, year: parsed.year,
            revision: parsed.revision, code: parsed.code,
  ...
  async function handleImport() {
    ...
    try {
      ...
      const res = await api.post('/drawings/import-bulk', { documents });
      setResult(res.data);
      ...
      <div className="modal" style={{ maxWidth: 780, width: '95vw', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 8, padding: '10px 14px', fontSize: '0.8rem', color: '#1D4ED8', lineHeight: 1.6 }}>
            ℹ️ Selecione o arquivo <strong>NUMERAÇÃO DE DESENHOS .docx</strong>. Colunas detectadas automaticamente por cabeçalho (Nº Desenho, Solicitante, Data, Assunto). A usina é extraída do próprio código quando possível. Solicitantes são correlacionados com os usuários do sistema.
          </div>
            ...
            <div>
              <div style={{ background: '#FEF3C7', border: '1px solid #F59E0B', borderRadius: 8, padding: '10px 14px', marginBottom: 10 }}>
                <div style={{ fontSize: '0.82rem', color: '#92400E', fontWeight: 700 }}>
                  ⚠️ {unresolvedNames.length} solicitante(s) não encontrado(s) — selecione manualmente:
                </div>
              </div>
                  ...
                  <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#FFFBEB', borderRadius: 8, border: '1px solid #FDE68A' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <span style={{ fontSize: '0.68rem', color: '#B45309', marginLeft: 6 }}>
                        ({preview.filter(r => r._originalResponsible === name).length} desenho{preview.filter(r => r._originalResponsible === name).length !== 1 ? 's' : ''})
                      </span>
                    </div>
                    <AppSelect
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
                  <thead>
                    <tr style={{ background: '#001F5B', position: 'sticky', top: 0 }}>
                      {['Código', 'Usina', 'Solicitante', 'Data', 'Assunto'].map(h => (
                  ...
                  <tbody>
                    {preview.map((r, i) => (
                      <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#F8FAFC', borderBottom: '1px solid #F1F5F9' }}>
                        ...
                        <td style={{ padding: '7px 10px', fontSize: '0.72rem', color: r.plant?.length ? '#334155' : '#CBD5E1', whiteSpace: 'nowrap' }}>{r.plant?.length ? r.plant.join(', ') : '—'}</td>
        ...
        <div className="modal-footer" style={{ flexShrink: 0 }}>
          ...
          {preview.length > 0 && !result && (
            <button className="btn btn-primary" onClick={handleImport} disabled={importing}>
              {importing ? 'Importando...' : `📥 Importar ${preview.length} desenhos`}
            </button>
          )}
        </div>
      </div>
...
export default function ControleDesenhosPage() {
  const { user }    = useAuth();
  const pageReadOnly = usePageAccess('drawings') === 'viewer';
  const { toast }   = useToast();
  ...
  const { widths: colWidths, handleResizeStart } = useColumnWidths(DRAW_COL_WIDTHS);
  ...
  const fetchDocs = useCallback(async () => {
    ...
    try {
      ...
      const [docsRes, statsRes] = await Promise.all([
        ...
        api.get(`/drawings/stats${params}`),
      ]);
      setDocs(Array.isArray(docsRes.data) ? docsRes.data : []);
      setStats(statsRes.data);
    } catch { toast('Erro ao carregar desenhos.', 'error'); }
  ...
  const fetchNextSeq = async () => {
    try { const r = await api.get(`/drawings/next-sequence?year=${CURRENT_YEAR_SHORT}`); setNextSeq(r.data.next); } catch {}
  };
  ...
  const exportHTML = useCallback(() => {
    ...
    a.download = `CTG_Desenhos_${CURRENT_YEAR}.html`;
  ...
  const exportExcel = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${base}/export/drawings${yearParam}`, { credentials: 'include' });
      if (!res.ok) throw new Error('Falha na exportação');
      const blob = await res.blob();
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      const yearLabel = yearFilter || CURRENT_YEAR;
      link.download = `CTG_Desenhos_${yearLabel}.xlsx`;
      link.click();
  ...
  useEffect(() => {
    window._exportDrawingsExcel = exportExcel;
    window._exportDrawingsHTML  = exportHTML;
    const onImport = () => setImportDocxModal(true);
    window.addEventListener('import-drawings-docx', onImport);
    return () => {
      delete window._exportDrawingsExcel;
      delete window._exportDrawingsHTML;
      window.removeEventListener('import-drawings-docx', onImport);
    };
  ...
  const statusChartData = STATUSES.map(s => {
    const itemsForStatus = statusChartDocs.filter(d=>d.status===s.value);
    return { label:s.value, value:itemsForStatus.length, color:s.color, filterKey: s.value,
      tooltipBreakdowns: [
        { title:'Tipo',  items: DRAWING_TYPES.map((t,i) => ({ label: t.value, value: itemsForStatus.filter(d=>d.type===t.value).length, color: TYPE_COLORS[i%TYPE_COLORS.length] })) },
  ...
  const plantChartData  = ALL_PLANTS.map(p => {
    ...
    return { label: PLANT_SIGLAS[p] || p, fullName: p, value:itemsForPlant.length, color:'#0066B3', filterKey: p,
      tooltipBreakdowns: [
        { title:'Tipo',   items: DRAWING_TYPES.map((t,i) => ({ label: t.value, value: itemsForPlant.filter(d=>d.type===t.value).length, color: TYPE_COLORS[i%TYPE_COLORS.length] })) },
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16, padding:'0 2px' }}>
      ...
      <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'stretch', position:'relative' }}>
        <div style={{ flex:'3 1 420px', minWidth:220, display:'flex' }}>
          <VBarChart
            title="Desenhos por Usina"
            data={plantChartData}
      ...
      <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
        ...
        <button onClick={() => setMyDocsOnly(v=>!v)} style={{
          ...
          {myDocsOnly ? 'Todos os desenhos' : 'Meus desenhos'}
        ...
        <div style={{ display:'flex', gap:6, alignItems:'center', flex:1, background:'#fff', border:'1px solid #E2E8F0', borderRadius:8, padding:'7px 12px', minWidth:0 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Buscar código, solicitante, assunto, autor..."
          ...
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} style={selStyle(!!typeFilter)}>
            ...
            {DRAWING_TYPES.map(t => <option key={t.value} value={t.value}>{t.value}</option>)}
          </select>
          <Div/>
          ...
          <Div/>
          <span style={{ fontSize:'0.72rem', color:'#94A3B8', whiteSpace:'nowrap', flexShrink:0 }}>{groups.length} grupos / {filtered.length} desenhos</span>
        </div>
      </div>
      ...
      <div style={{ background:'#fff', border:'1px solid #E2E8F0', borderRadius:10, overflow:'hidden', maxHeight:'calc(100vh - 460px)', minHeight:200, overflowY:'auto' }}>
        {loading ? (
          <div style={{ padding:48, textAlign:'center', color:'#94A3B8' }}>
            <div className="spinner" style={{ margin:'0 auto 12px' }}/>Carregando desenhos...
          </div>
        ) : groups.length === 0 ? (
          <div style={{ padding:48, textAlign:'center' }}>
            <div style={{ fontSize:'2rem', marginBottom:10 }}>📐</div>
            <div style={{ fontSize:'0.9rem', color:'#64748B', fontWeight:600 }}>
              {myDocsOnly ? 'Você não é autor de nenhum desenho' : 'Nenhum desenho encontrado'}
            </div>
            <button onClick={openNew} style={{ marginTop:12, padding:'8px 18px', border:'none', borderRadius:8, background:'#001F5B', color:'#fff', fontSize:'0.82rem', fontWeight:700, cursor:'pointer' }}>+ Registrar Desenho</button>
          </div>
        ) : (
          <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed', minWidth: DRAW_COL_WIDTHS.reduce((s,w)=>s+w,0) }}>
            <colgroup>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
                          {hasRevisions && (
                            <button onClick={e => { e.stopPropagation(); setExpandedGroup(groupOpen ? null : key); }}
                              title={groupOpen ? 'Recolher revisões' : `Ver ${items.length} versões deste desenho`}
                        ...
                        <div style={{ display:'flex', gap:5, alignItems:'center' }}>
                          {isAuthor(latest) && (
                            <>
                              ...
                              <ActionBtn color="#475569" onClick={() => openEdit(latest)} tooltip="Editar Desenho">✏️</ActionBtn>
                            </>
                          )}
                          ...
                          <td style={{ ...TD }} onClick={e => e.stopPropagation()}>
                            {isAuthor(rev) && (
                              <div style={{ display:'flex', gap:5 }}>
                                ...
                                <ActionBtn color="#475569" onClick={() => openEdit(rev)} tooltip="Editar Desenho">✏️</ActionBtn>
                              </div>
                            )}
                          </td>
                        </tr>
...
/* ─── DocDetail — painel expandido de um desenho ────────────────────────────── */
